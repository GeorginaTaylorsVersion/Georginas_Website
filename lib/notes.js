...
export function getTerms() {
  return fs.readdirSync(NOTES_DIR).filter(name => {
    const termPath = path.join(NOTES_DIR, name);
    if (!fs.statSync(termPath).isDirectory()) return false;
    const hasCourse = fs.readdirSync(termPath).some(child =>
      fs.statSync(path.join(termPath, child)).isDirectory()
    );
    return hasCourse;
  });
}
